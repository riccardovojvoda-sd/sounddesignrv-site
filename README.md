...
- Quando si aggiunge una lingua: cartella `src/<lang>/` con `<lang>.json` (`{ "lang": "<lang>" }`), `progetti.json` con tag `progetti_<lang>` (la collezione nasce da sola in `.eleventy.js`), blocco in `testi.js`, `faq-<lang>.js` agganciato in `faq.js`, riga in `llms.njk` e redirect in `.htaccess`. `ORDINE_LINGUE` in `.eleventy.js` fissa l'ordine delle alternative (it, en, fr, es).
